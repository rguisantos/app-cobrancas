...
export default function RotasGerenciarScreen() {
  ...
  const handleSalvar = async () => {
    if (!descricao.trim()) { Alert.alert('Erro', 'Digite a descrição da rota'); return; }
    setSalvando(true);
    try {
      const resultado = await salvarRota({ id: editando?.id, descricao: descricao.trim(), status: 'Ativo' });
      if (resultado) {
  ...
  const handleExcluir = (rota: Rota) => {
    Alert.alert(
      'Excluir Rota',
      `Excluir "${rota.descricao}"?\n\nAtenção: clientes associados a esta rota ficarão sem rota.`,
      [
  ...
  const renderItem = ({ item }: { item: Rota }) => (
    <View style={s.card}>
      <View style={s.cardLeft}>
        <View style={s.iconBox}><Ionicons name="map" size={20} color="#2563EB" /></View>
        <View style={s.cardText}>
          <Text style={s.cardDescricao}>{item.descricao}</Text>
          <Text style={[s.cardStatus, { color: item.status === 'Ativo' ? '#16A34A' : '#DC2626' }]}>
            {item.status}
          </Text>
        </View>
      </View>
...
const s = StyleSheet.create({
  ...
  input:        { backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 12, padding: 14, fontSize: 16, color: '#1E293B', marginBottom: 12 },
